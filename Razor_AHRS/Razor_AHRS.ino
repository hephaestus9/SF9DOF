...
#define ACCEL_X_MIN ((float) -214)
#define ACCEL_X_MAX ((float) 272)
#define ACCEL_Y_MIN ((float) -269)
#define ACCEL_Y_MAX ((float) 276)
#define ACCEL_Z_MIN ((float) -243)
#define ACCEL_Z_MAX ((float) 276)

// Magnetometer (standard calibration mode)
// "magn x,y,z (min/max) = X_MIN/X_MAX  Y_MIN/Y_MAX  Z_MIN/Z_MAX"
#define MAGN_X_MIN ((float) -594)
#define MAGN_X_MAX ((float) 487)
#define MAGN_Y_MIN ((float) -539)
#define MAGN_Y_MAX ((float) 411)
#define MAGN_Z_MIN ((float) -466)
#define MAGN_Z_MAX ((float) 525)
...
#define GYRO_AVERAGE_OFFSET_X ((float) 197.93)
#define GYRO_AVERAGE_OFFSET_Y ((float) 51.3)
#define GYRO_AVERAGE_OFFSET_Z ((float) 86.4)
...
float xhat[1] = {3};
float P[1] = {1};
float controlVector[1] = {0};
float current_state_estimate_accel_X = xhat[0];
float current_state_estimate_accel_Y = xhat[0];
float current_state_estimate_accel_Z = xhat[0];
float current_state_estimate_magnetom_X = xhat[0];
float current_state_estimate_magnetom_Y = xhat[0];
float current_state_estimate_magnetom_Z = xhat[0];
float current_state_estimate_gyro_X = xhat[0];
float current_state_estimate_gyro_Y = xhat[0];
float current_state_estimate_gyro_Z = xhat[0];
float current_prob_estimate_accel_X = P[0];
float current_prob_estimate_accel_Y = P[0];
float current_prob_estimate_accel_Z = P[0];
float current_prob_estimate_magnetom_X = P[0];
float current_prob_estimate_magnetom_Y = P[0];
float current_prob_estimate_magnetom_Z = P[0];
float current_prob_estimate_gyro_X = P[0];
float current_prob_estimate_gyro_Y = P[0];
float current_prob_estimate_gyro_Z = P[0];
float accelX = 0.0;
float accelY = 0.0;
float accelZ = 0.0;
float magnetomX = 0.0;
...
float gyroY = 0.0;
float gyroZ = 0.0;
float prevKgyroX = xhat[0];
float prevKgyroY = xhat[0];
float prevKgyroZ = xhat[0];
...
void compensate_sensor_errors() {
    ...
    accelX = current_state_estimate_accel_X;
    current_state_estimate_accel_X, current_prob_estimate_accel_X = Step(controlVector, accel[0], current_state_estimate_accel_X, current_prob_estimate_accel_X);
    accel[0] = (accelX - ACCEL_X_OFFSET) * ACCEL_X_SCALE;
    
    accelY = current_state_estimate_accel_Y;
    current_state_estimate_accel_Y, current_prob_estimate_accel_Y = Step(controlVector, accel[1], current_state_estimate_accel_Y, current_prob_estimate_accel_Y);
    accel[1] = (accelY - ACCEL_Y_OFFSET) * ACCEL_Y_SCALE;
    
    accelZ = current_state_estimate_accel_Z;
    current_state_estimate_accel_Z, current_prob_estimate_accel_Z = Step(controlVector, accel[2], current_state_estimate_accel_Z, current_prob_estimate_accel_Z);
    accel[2] = (accelZ - ACCEL_Z_OFFSET) * ACCEL_Z_SCALE;
...
#else
    magnetomX = current_state_estimate_magnetom_X;
    current_state_estimate_magnetom_X, current_prob_estimate_magnetom_X = Step(controlVector, magnetom[0], current_state_estimate_magnetom_X, current_prob_estimate_magnetom_X);
    magnetom[0] = (magnetomX - MAGN_X_OFFSET) * MAGN_X_SCALE;
    
    magnetomY = current_state_estimate_magnetom_Y;
    current_state_estimate_magnetom_Y, current_prob_estimate_magnetom_Y = Step(controlVector, magnetom[1], current_state_estimate_magnetom_Y, current_prob_estimate_magnetom_Y);
    magnetom[1] = (magnetomY - MAGN_Y_OFFSET) * MAGN_Y_SCALE;
    
    magnetomZ = current_state_estimate_magnetom_Z;
    current_state_estimate_magnetom_Z, current_prob_estimate_magnetom_Z = Step(controlVector, magnetom[2], current_state_estimate_magnetom_Z, current_prob_estimate_magnetom_Z);
    magnetom[2] = (magnetomZ - MAGN_Z_OFFSET) * MAGN_Z_SCALE;
#endif

    // Compensate gyroscope error
    //if (gyro[0] == 1.0) gyro[0] = prevKgyroX;
    gyroX = current_state_estimate_gyro_X;
    prevKgyroX = gyroX;
    current_state_estimate_gyro_X, current_prob_estimate_gyro_X = Step(controlVector, gyro[0], current_state_estimate_gyro_X, current_prob_estimate_gyro_X);
    gyro[0] = gyro[0] - GYRO_AVERAGE_OFFSET_X;
    
    //if (gyro[1] == 1.0) gyro[1] = prevKgyroY;
    gyroY = current_state_estimate_gyro_Y;
    prevKgyroY = gyroY;
    current_state_estimate_gyro_Y, current_prob_estimate_gyro_Y = Step(controlVector, gyro[1], current_state_estimate_gyro_Y, current_prob_estimate_gyro_Y);
    gyro[1] = gyro[1] - GYRO_AVERAGE_OFFSET_Y;
    
    //if (gyro[2] == 1.0) gyro[2] = prevKgyroZ;
    gyroZ = current_state_estimate_gyro_Z;
    prevKgyroZ = gyroZ;
    current_state_estimate_gyro_Z, current_prob_estimate_gyro_Z = Step(controlVector, gyro[2], current_state_estimate_gyro_Z, current_prob_estimate_gyro_Z);
